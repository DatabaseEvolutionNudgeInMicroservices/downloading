            ...
            if (element !== undefined
                ...
                try {
                    ...
                    if (urlParts.length === 2) {
                        ...
                        exec(
                            `git -C ${process.cwd()}${FILE_SYSTEM_SEPARATOR}${TEMP_FOLDER_NAME}${FILE_SYSTEM_SEPARATOR} -c credential.helper= -c core.longpaths=true clone ${repositoryUrl}.git ${folderName}`,
                            (cloneError) => {
                                if (cloneError) {
                                    reject(new DownloadFail(cloneError.message));
                                } else {
                                    exec(
                                        `git -C ${process.cwd()}${FILE_SYSTEM_SEPARATOR}${TEMP_FOLDER_NAME}${FILE_SYSTEM_SEPARATOR}${folderName} checkout ${hash}`,
                                        (checkoutError) => {
                                            if (checkoutError) {
                                                reject(new DownloadFail(checkoutError.message));
                                            } else {
                                                const denimFilePath = `${process.cwd()}${FILE_SYSTEM_SEPARATOR}${TEMP_FOLDER_NAME}${FILE_SYSTEM_SEPARATOR}${folderName}${FILE_SYSTEM_SEPARATOR}denim`
