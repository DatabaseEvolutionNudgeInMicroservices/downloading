...
class ArchiverZip extends Archiver {
   ...
   */
  archiveByList(list, destination) {
    return new Promise((resolve, reject) => {
      if (list !== undefined && list !== null && list.length !== 0) {
        if (
          list.every((directory) =>
            fs.existsSync(
              `${process.cwd()}${FILE_SYSTEM_SEPARATOR}${TEMP_FOLDER_NAME}${FILE_SYSTEM_SEPARATOR}${destination}${FILE_SYSTEM_SEPARATOR}${directory}`
            )
          )
        ) {
          const timestamp = new Date().toISOString().replace(/[:.]/g, '-')
          const zipName = `${timestamp}.zip`
          const zipPath = `${process.cwd()}${FILE_SYSTEM_SEPARATOR}${TEMP_FOLDER_NAME}${FILE_SYSTEM_SEPARATOR}${destination}${FILE_SYSTEM_SEPARATOR}${zipName}`
          ...
          list.forEach((directory) => {
            let path = `${process.cwd()}${FILE_SYSTEM_SEPARATOR}${TEMP_FOLDER_NAME}${FILE_SYSTEM_SEPARATOR}${destination}${FILE_SYSTEM_SEPARATOR}${directory}`
