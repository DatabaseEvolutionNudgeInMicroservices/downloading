...
class CleanerFile extends Cleaner {
    ...
    /**
     * Deletes all the files and directories in the TEMP directory.
     */
    cleanAll() {
        return new Promise(async (resolve, reject) => {
            let tempDirectory = `${process.cwd()}${FILE_SYSTEM_SEPARATOR}${TEMP_FOLDER_NAME}`;
            for (const element of fs.readdirSync(tempDirectory)) {
                await this.cleanByElement(element);
            }
            resolve(true);
        });
    }
